...
python3 /src/CALDERA/bin/caldera-script -l ${data} -o ${out} -v -t 6 \
    -P /Raw/Amikacin/PA_pop_kmeans2.txt -s 7 --alpha=0 --Lmax 2000 >> ${out}/caldera.out 2>&1
...
do 
    ...
    #     -P /Raw/Amikacin/PA_pop_kmeans2.txt -s 7 --alpha=${alpha} \
    #     --Lmax 2000 >> ${out}/caldera.out 2>&1
    # DBGWAS -strains ${raw}/pseudomonas_aeruginosa_full_dataset/strains -skip1 -skip2 \
    #     -nb-cores 2 -output ${out}/Caldera_S7_${alpha} -caldera \
    #     -SFF p1.0 -nh 2 >> ${out}/caldera.out 2>&1
    # docker-machine scp -r $machineName:/tmp/data/Amikacin/Caldera_S7_${alpha}/step2/nodes_to_css ../Datasets/Amikacin/Tries/Caldera_S7_${alpha}/step2/nodes_to_css
    # echo "" >> ${out}/all_steps.out
...
Rscript --no-save --verbose /Analysis/Amikacin/Comp_Results.R \
