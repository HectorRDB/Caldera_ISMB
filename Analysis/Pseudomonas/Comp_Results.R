...
# Load input
aac6 <- read_lines(here("Raw", "Pseudomonas", "aac-significant-unitigs.txt"))
plasmid <- read_lines(here("Raw", "Pseudomonas", "plasmid-all-unitigs.txt"))

# Analyze function
metrics <- function(loc, alpha) {
  print(loc)
  file <- here("Output", "Pseudomonas", loc, "textualOutput", "all_comps_nodes_info.tsv")
  if (!file.exists(file)) {
...
locs <- list.dirs(here("Output", "Pseudomonas"), recursive = FALSE, full.names = FALSE) %>%
  str_subset(loc)
...
write_csv(res,  here("Output", "Pseudomonas", paste0(loc, "results.csv")))
