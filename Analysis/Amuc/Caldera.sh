...
R CMD BATCH /Analysis/Amuc/clean_metadata.R ${out}/clean_metadata.out
# Get genomes
...
esearch -db protein -query "refseq[filter] AND txid239935[Organism:exp]" | efetch -format fasta > ${raw}/amuc.refseq.sequences
cp ${raw}/amuc.refseq.sequences ${out}

# Running CALDERA
echo "Running DBGWAS" >> ${out}/all_steps.out
cd /
DBGWAS -strains ${raw}/strains -SFF q0.05 -nh 2\
    -nb-cores 4 -output ${data} -pt-db ${raw}/amuc.refseq.sequences \
...
DBGWAS -strains ${raw}/strains -skip1 -skip2 -caldera \
    -pt-db ${raw}/amuc.refseq.sequences -output ${out}/CALDERA > ${out}/viz.out 2>&1
