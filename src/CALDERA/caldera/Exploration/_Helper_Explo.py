...
def messages(are_Testable, N, Keep, Not_Too_Large, k):
    ...
    if not_keep == 0:
        ...
        fraction_size = 100 * round(
                              (sum(Not_Too_Large == False)) / not_keep, 2)
...
def start(nodes, Pop, Patterns, Neighbours, Lengths, TH, n1s, n2s):
    ...
    """
    R = [create_Graph(s, Pop, Patterns, Neighbours, Lengths, TH, n1s, n2s)
         for s in nodes]
    R = np.array([S for S in R if S.closed and not S.to_prune], dtype = graph)
    return R

def create_Graph(s, Pop, Patterns, Neighbours, Lengths, TH, n1s, n2s):
    ...
    """
    S = graph()
    S.new_graph(s, Pop, Patterns, Neighbours, Lengths, TH, n1s, n2s)
    return S

def expand(Ss, Pop, Patterns, Neighbours, Lengths, TH, n1s, n2s):
    """Find all parents
...
class structure:
    ...
    def ns(self):
        """
        Compute the margins of all tables when summing over phenotypes, for each group
        """
    ...
    def offload(self, verbose):
        """
        For a given structure, write the large files to disk to limit
        memory requirements
        """
