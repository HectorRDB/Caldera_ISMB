################################################################################
# Current way to store the subgrapphs, i.e a list of nodes, a pattern, a set of
# neighbours.
################################################################################
...
################################################################################
# Known limitations (might adress in future versions):
# - there is a bug if we reach the full graph
# - there is a bug if we have no neighbors (i.e. we reach the full local graph)
################################################################################
...
class graph:
    ...
    Slots:
        ...
        pval: the subgraph's minimal p-value
    ...
    def new_graph(self, node, Pop, Patterns, Neighbours, Lengths, TH, n1s, n2s, old_Env):
        """Creating the closure of the subgraph {node}
        ...
        self.compute_envelope(Pop, n1s, n2s, old_Env)
        ## List of neighoring nodes that can be added
    ...
    def Childrens(self, Pop, Patterns, Neighbours, Lengths, TH, n1s, n2s, old_Env,
                  L_Stop = 10 ** 5):
        ...
        # Second is for the second stage
        children_indexes = [index for i, index in enumerate(indexes) if
                            iSs[i] > self.iS and not self.ys[iSs[i]]]
        childrens = enumerate_childrens(self, children_indexes, Patterns,
                                        Neighbours, Lengths, TH, Pop, n1s, n2s,
                                        L_Stop, old_Env)
    ...
    def add_parent_node(self, node, Patterns, common, Neighbours, Lengths, iS):
        ...
        self.direct = True
        self.diff_patterns = np.empty((), dtype = np.bool_)
        return
    ...
    def add_sibling_node(self, node, pattern, common, Neighbours, Lengths,
        ...
        diff_positions = np.array([i for i, obj in enumerate(self.diff_items)
                                   if obj in diff_items], dtype = np.int)
        if self.direct:
            self.diff_patterns = new_diff_patterns[:,diff_items]
            self.direct = False
        else:
...
def enumerate_siblings(S, siblings_indexes, siblings_closure, Patterns,
                       Neighbours, Lengths, TH, Pop, n1s, n2s, L_Stop, old_Env):
